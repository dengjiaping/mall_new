...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        LogUtils.i("isPullToRefreshEnabled():" + isPullToRefreshEnabled());
        ...
        LogUtils.i("mIsBeingDragged1:" + mIsBeingDragged);
        ...
        LogUtils.i("mIsBeingDragged2:" + mIsBeingDragged);

        if (action != MotionEvent.ACTION_DOWN && mIsBeingDragged) {
            return true;
        }
        LogUtils.i("action != MotionEvent.ACTION_DOWN:" + (action != MotionEvent.ACTION_DOWN) + "  mIsBeingDragged:" + mIsBeingDragged);

        switch (action) {
            case MotionEvent.ACTION_MOVE: {

                LogUtils.i("MotionEvent.ACTION_MOVE !mScrollingWhileRefreshingEnabled:" + (!mScrollingWhileRefreshingEnabled) + "  isRefreshing():" + isRefreshing());
                // If we're refreshing, and the flag is set. Eat all MOVE events
                if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
                    return true;
                }
                LogUtils.i("MotionEvent.ACTION_MOVE isReadyForPull():" + isReadyForPull());
                if (isReadyForPull()) {
                    ...
                    LogUtils.i("MotionEvent.ACTION_MOVE mIsBeingDragged:" + mIsBeingDragged);
                }
                break;
            }
            case MotionEvent.ACTION_DOWN: {
                LogUtils.i("MotionEvent.ACTION_DOWN isReadyForPull():" + isReadyForPull());
                if (isReadyForPull()) {
                    mLastMotionY = mInitialMotionY = event.getY();
                    mLastMotionX = mInitialMotionX = event.getX();
                    mIsBeingDragged = false;
                }
                LogUtils.i("MotionEvent.ACTION_DOWN mIsBeingDragged:" + mIsBeingDragged);
                break;
            }
        }
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        LogUtils.i("mScrollingWhileRefreshingEnabled:" + mScrollingWhileRefreshingEnabled + "     isRefreshing" + isRefreshing());
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (mIsBeingDragged) {
                    mIsBeingDragged = false;
                    LogUtils.i("setState(State.REFRESHING, true)");
                    ...
                    if (isRefreshing()) {
                        LogUtils.i("MotionEvent.ACTION_UP smoothScrollTo(0)");
                        smoothScrollTo(0);
                        return true;
                    }
    ...
    protected LinearLayout getRefreshableViewWrapper() {
    ...
    private void addRefreshableView(Context context, T refreshableView) {
        mRefreshableViewWrapper = new LinearLayout(context);
        mRefreshableViewWrapper.addView(refreshableView, new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT));

        addViewInternal(mRefreshableViewWrapper, new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT));
    }
    ...
    private void callRefreshListener() {
        if (null != mOnRefreshListener) {
            ...
            if (mCurrentMode == Mode.PULL_FROM_START) {
                ...
                afterPullDownRefresh();
            }
        }
    }

    public void afterPullDownRefresh() {
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                ...
                case PENGSI:
                    return new PengsiLoadingLayout(context, mode, scrollDirection, attrs);
                case DYNAMIC:
                    return new DynamicRotateLoadingLayout(context, mode, scrollDirection, attrs);
            }
        }
    }
    ...
    public static interface OnRefreshListener<V extends View> {
        ...
        /**
         * onRefreshData will be called for both a Pull from start, and Pull from
         * end
         */
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mStartTime == -1) {
                ...
                LogUtils.i("setHeaderScroll(mCurrentY)");
    ...
    /**
     * 获取自动刷新header的高度（如果没有测量出来）
     *
     * @return
     */
    private int getRefreshSelf() {
        return DensityUtils.dip2px(12) * 3 + DensityUtils.sp2px( 14) + DensityUtils.sp2px( 16);
    }
