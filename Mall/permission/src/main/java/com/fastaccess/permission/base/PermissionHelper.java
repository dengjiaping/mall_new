...
/**
 * 修改onRequestPermissionsResult方法中onPermissionReallyDeclined的调用时机
 * by zfl
 */
    ...
    @Override public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_PERMISSIONS) {
            if (verifyPermissions(grantResults)) {
                ...
                List<String> permissionReallyDeclined = new ArrayList<>();//被拒绝且勾选不再提示的permission
                ...
                for (String permissionName : declinedPermissions) {
                    if (permissionName != null && !isExplanationNeeded(permissionName)) {
                        ...
                        permissionReallyDeclined.add(permissionName);
                    }
                }
                if ( !permissionReallyDeclined.isEmpty() ){
                    permissionCallback.onPermissionReallyDeclined(permissionReallyDeclined);
                }

                if (permissionReallyDeclined.size() == 0) {//没有勾选不再提示选项
                    if (forceAccepting) {
